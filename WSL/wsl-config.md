...
> [!NOTE]
> Если у вас возникли проблемы с определением имени дистрибутива, см. [Список дистрибутивов](#list-distributions) для команды, чтобы получить список официальных названий установленных дистрибутивов. 
...
|метаданные | Добавляются ли метаданные в файлы Windows для поддержки системных разрешений Linux | Включено
...
| ключ | value | default | HDInsight|
|:----|:----|:----|:----|
| Включено | Логический | `true` | Установка этого ключа определяет, будет ли WSL поддерживать запуск процессов Windows. |
...
| Память | размер; | 50% от общего объема памяти в Windows или 8 ГБ, в зависимости от того, что меньше. в сборках до 20175:80% от общего объема памяти в Windows | Объем памяти, назначаемый виртуальной машине WSL 2. |
