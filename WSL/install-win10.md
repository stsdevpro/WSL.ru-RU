...
> [!NOTE]
> Чтобы использовать новую команду `wsl --install` и пропустить описанные ниже шаги 1–6, необходимо зарегистрироваться в [Программе предварительной оценки Windows](https://insider.windows.com/getting-started) и установить предварительную сборку Windows 10 (сборка ОС 20262 или более поздней версии). 
>
> Как только предварительная сборка будет установлена, откройте окно командной строки с правами администратора и выполните команду `wsl --install`. Это позволит автоматически включить необязательные компоненты WSL и платформы виртуальных машин, скачать и установить последнюю версию ядра Linux, задать WSL 2 в качестве значения по умолчанию и скачать Ubuntu. При этом дистрибутив можно изменить, например с помощью команды `wsl --install -d Debian`. Чтобы просмотреть список доступных дистрибутивов Linux, введите `wsl --list --online`. Когда команда будет выполнена, отобразится запрос перезагрузки. После перезагрузки дистрибутив Linux (Ubuntu по умолчанию) завершает установку и открывает командную строку Linux, чтобы вы могли приступить к работе. Затем вы можете перейти к [шагу 7 по настройке нового дистрибутива](./install-win10.md#step-7---set-up-a-new-distribution).

### <a name="install-steps"></a>Инструкции по установке

- Откройте командное окно от имени администратора.
- Выполнить `wsl.exe --install`
- Перезагрузите компьютер при необходимости и наличии такого указания в команде.
- После перезагрузки установка завершится и все будет готово к использованию WSL.

Будет установлен дистрибутив Ubuntu. Можно установить и другие дистрибутивы, передав соответствующие аргументы. Например, введите `wsl --install -d Debian`, чтобы установить Debian. Выполните команду `wsl --list --online`, чтобы отобразить список доступных дистрибутивов. 
...
В Терминале Windows можно использовать несколько вкладок (чтобы быстро переходить между несколькими командными строками Linux, командной строкой Windows, PowerShell, Azure CLI и пр.), создавать пользовательские сочетания клавиш (для открытия и закрытия вкладок, копирования и вставки и пр.), а также применять функцию поиска и пользовательские темы (цветовые схемы, стили и размеры шрифтов, а также фоновое изображение, размытие и прозрачность). [Подробнее.](/windows/terminal)

[Установка Терминала Windows](/windows/terminal/get-started)
